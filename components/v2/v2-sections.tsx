...
import { V2_FOOTER_LINKS, V2_IMAGES } from "@/lib/v2-content";

export function V2Contact() {
  const { open } = useContactForm();

  return (
    <section id="v2-contact" className="relative overflow-hidden bg-[#0d1322] section-pad">
      ...
      <div className="container-luxury relative text-center">
        <h2 className="v2-heading-light mx-auto max-w-3xl">
        ...
        <button type="button" onClick={open} className="v2-btn-primary mt-12">
          Обсудить проект
        </button>
      </div>
    </section>
    ...
    <footer className="border-t border-white/10 bg-[#080b15] py-16">
      <div className="container-luxury grid grid-cols-1 gap-12 tablet:grid-cols-12">
        <div className="tablet:col-span-4">
          <p className="v2-footer-brand">VERANDARU</p>
        ...
        <div className="grid grid-cols-2 gap-8 tablet:col-span-5">
          <div>
            <p className="v2-footer-col-title">О нас</p>
            <ul className="mt-4 space-y-2">
              {V2_FOOTER_LINKS.about.map((l) => (
                <li key={l.label}>
                  <a href={l.href} className="v2-footer-link">
                    {l.label}
                  </a>
                </li>
              ))}
          ...
          <div>
            <p className="v2-footer-col-title">Работа</p>
            <ul className="mt-4 space-y-2">
              {V2_FOOTER_LINKS.work.map((l) => (
                <li key={l.label}>
                  <a href={l.href} className="v2-footer-link">
                    {l.label}
                  </a>
        ...
        <div className="tablet:col-span-3">
          <p className="v2-footer-col-title">Связаться</p>
          <button type="button" onClick={open} className="v2-footer-link mt-4 block">
            Обсудить проект
          </button>
        </div>
      </div>
